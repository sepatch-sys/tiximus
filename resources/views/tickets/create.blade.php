<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg">
        ...
        <form action="{{ route('tickets.store') }}" method="POST" class="space-y-4">
            ...
            {{-- Category Province --}}
            <div>
                <label for="category_province_id" class="block text-sm font-medium text-gray-700">Category Province</label>
                <select name="category_province_id" id="category_province_id"
                    ...
                    @foreach($categoryProvinces as $province)
                        <option value="{{ $province->id }}" {{ old('category_province_id') == $province->id ? 'selected' : '' }}>
            ...
            {{-- Category Tourism --}}
            <div>
                <label for="category_tourism_id" class="block text-sm font-medium text-gray-700">Category Tourism</label>
                <select name="category_tourism_id" id="category_tourism_id"
                    ...
                    @foreach($categoryTourisms as $tourism)
                        <option value="{{ $tourism->id }}" {{ old('category_tourism_id') == $tourism->id ? 'selected' : '' }}>
            ...
            <div class="flex items-center space-x-2 mt-4">
                <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition flex items-center space-x-2">
                    <i class="fas fa-save"></i> <span>Save</span>
                </button>
                <a href="{{ route('tickets.index') }}" class="bg-gray-400 text-white px-4 py-2 rounded-lg hover:bg-gray-500 transition flex items-center space-x-2">
